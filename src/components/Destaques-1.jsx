...
const HighlightCard = ({ className = "", discount, img, title }) => {
    return (
        <div className={`bg-[#D8E3F2] rounded-lg relative p-[20px] md:p-0 ${className}`}>
            <div className="flex flex-col text-left w-[194px] gap-2.5 md:gap-0 md:mt-[34px] md:ml-[30px] relative z-[1]">
...
export default function Destaques1() {
    ...
    return (
        <div className="flex flex-col my-10 md:my-[38px_0_100px] gap-2.5">
            <h3 className={`text-dark-gray-2 self-start ${window.innerWidth >= 768 ? "c-text-large" : "c-text-small"} bold`}>
                Coleções em Destaque
            </h3>
            <div className="!grid grid-cols-[repeat(1,338px)] grid-rows-3 gap-y-[10px] md:grid-cols-[repeat(3,405px)] md:grid-rows-[repeat(1,251px)] md:gap-x-[12px] ">
                <HighlightCard
